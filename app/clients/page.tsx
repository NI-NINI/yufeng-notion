...
const CONTACT_FIELDS = [1,2,3,4] // 最多4個聯絡窗口
...
const getContacts = (c: Partial<Client>): Contact[] => {
  const cs: Contact[] = []
  CONTACT_FIELDS.forEach(i => {
    const name = (c as any)[`contact${i}Name`] || ''
    const phone = (c as any)[`contact${i}Phone`] || ''
    const email = (c as any)[`contact${i}Email`] || ''
    if (name || phone || email || i <= 1) cs.push({ name, phone, email })
  })
  return cs
}

const setContacts = (data: Partial<Client>, contacts: Contact[]): Partial<Client> => {
  const u: any = { ...data }
  CONTACT_FIELDS.forEach(i => {
    const c = contacts[i-1] || { name:'', phone:'', email:'' }
    u[`contact${i}Name`] = c.name
    u[`contact${i}Phone`] = c.phone
    u[`contact${i}Email`] = c.email
  })
  return u
}
...
const emptyClient = ():Partial<Client> => ({
  ...
  contact4Name:'', contact4Phone:'', contact4Email:'',
...
export default function ClientsPage() {
  ...
  const [contacts, setContacts2] = useState<Contact[]>([{name:'',phone:'',email:''}])
  ...
  const openEdit = (c: Client) => {
    ...
    setContacts2(cs.length > 0 ? cs : [{name:'',phone:'',email:''}])
    setModalOpen(true)
  }

  const updateContact = (i: number, field: keyof Contact, val: string) => {
    setContacts2(prev => prev.map((c, idx) => idx === i ? {...c, [field]: val} : c))
  }
  const addContact = () => {
    if (contacts.length < 4) setContacts2(prev => [...prev, {name:'',phone:'',email:''}])
  }
  const removeContact = (i: number) => {
    setContacts2(prev => prev.filter((_,idx) => idx !== i))
  }

  const save = async () => {
    setSaving(true)
    try {
      const payload = setContacts(editing, contacts)
      if (isNew.current) {
  ...
  const filtered = clients.filter(c => {
    ...
    if (giftFilter) { if (!(c as any)[giftFilter]) return false }
    return true
  })

  const firstContact = (c: Client) => {
    if (c.contact1Name || c.contact1Phone) return `${c.contact1Name}${c.contact1Phone?' · '+c.contact1Phone:''}`
    return ''
  }
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        <div className="scroll-area">
          {/* 表頭 */}
          <div style={{display:'grid',gridTemplateColumns:'30px 1fr 1fr 160px 60px',alignItems:'center',padding:'8px 16px',borderBottom:'1px solid var(--bd)',background:'var(--bgh)'}}>
            {['','客戶名稱','聯絡人','節日送禮',''].map((h,i)=>(
          ...
          {loading ? (
            ...
            <div key={c.id} className="client-row-wrap">
              <div className="client-hrow" style={{gridTemplateColumns:'30px 1fr 1fr 160px 60px'}}
                ...
                <span style={{fontSize:13,color:'var(--tx2)'}}>{firstContact(c)}</span>
              ...
              {expanded[c.id] && (
                <div className="client-expand open">
                  {/* 全部聯絡窗口 */}
                  {getContacts(c).filter(ct=>ct.name||ct.phone||ct.email).length > 0 && (
                    <div style={{display:'flex',gap:10,marginBottom:12,flexWrap:'wrap'}}>
                      {getContacts(c).filter(ct=>ct.name||ct.phone||ct.email).map((ct,i)=>(
                        <div key={i} style={{background:'var(--bgc)',border:'1px solid var(--bd)',borderRadius:6,padding:'8px 12px',fontSize:12,minWidth:160}}>
                          <div style={{fontWeight:700,marginBottom:3}}>聯絡窗口 {i+1}</div>
                          {ct.name && <div>{ct.name}</div>}
                          {ct.phone && <div style={{color:'var(--tx2)'}}>{ct.phone}</div>}
                  ...
                  {!casesMap[c.id] ? (
                    ...
                    <a key={cs.id} href={`/cases?highlight=${cs.id}`} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 12px',background:'var(--bgc)',border:'1px solid var(--bd)',borderRadius:6,marginBottom:5,fontSize:13,cursor:'pointer',textDecoration:'none'}}>
                      ...
                      <div style={{display:'flex',gap:10,alignItems:'center'}}>
                        ...
                        <span style={{fontFamily:'var(--m)',fontSize:12,color:'var(--tx2)'}}>
          ...
          <div className="mo-body">
            ...
            <div>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
                <span style={{fontSize:11,fontWeight:700,color:'var(--tx3)',textTransform:'uppercase',letterSpacing:'.05em'}}>聯絡窗口</span>
                {contacts.length < 4 && (
                  <button className="btn btn-sm" onClick={addContact}>＋ 新增窗口</button>
                )}
              </div>
              {contacts.map((ct, i) => (
                <div key={i} style={{border:'1px solid var(--bd)',borderRadius:6,padding:12,marginBottom:8,position:'relative'}}>
                  <div style={{fontSize:12,fontWeight:600,color:'var(--tx2)',marginBottom:8}}>
                    聯絡窗口 {i+1}
                    {i >= 1 && (
                      <button style={{marginLeft:8,fontSize:11,color:'var(--rose)',cursor:'pointer'}}
                        onClick={()=>removeContact(i)}>✕ 移除</button>
                    )}
                  </div>
            ...
            <div>
              <div style={{fontSize:11,fontWeight:700,color:'var(--tx3)',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:8}}>節日送禮</div>
              <div style={{display:'flex',gap:18,flexWrap:'wrap'}}>
                {GIFTS.map(g=>(
                  <label key={g.key} style={{display:'flex',alignItems:'center',gap:5,fontSize:13,cursor:'pointer'}}>
