...
const emptyContact=():Contact=>({name:'',dept:'',title:'',phone:'',ext:'',mobile:'',email:'',birthday:'',notes:'',giftMidAutumn:false,giftYearEnd:false,giftCalendar:false})
...
const contactsToPayload=(contacts:Contact[]):Record<string,any>=>{
  ...
  for(let i=1;i<=4;i++){
    ...
    out[`contact${i}Name`]=ct.name; out[`contact${i}Dept`]=ct.dept; out[`contact${i}Title`]=ct.title
    out[`contact${i}Phone`]=ct.phone; out[`contact${i}Ext`]=ct.ext; out[`contact${i}Mobile`]=ct.mobile
    out[`contact${i}Email`]=ct.email; out[`contact${i}Birthday`]=ct.birthday; out[`contact${i}Notes`]=ct.notes
...
const emptyClient=():Partial<Client>=>(
  {name:'',taxId:'',phone:'',fax:'',address:'',clientType:'',notes:'',giftMidAutumn:false,giftYearEnd:false,giftCalendar:false}
)

const fmtNo=(n:number|null)=>n!=null?`CL-${String(n).padStart(4,'0')}`:'—'

// 每個窗口的送禮標籤
const ContactGiftBadges=({ct}:{ct:Contact})=>(
  <div style={{display:'flex',gap:3,marginTop:3}}>
    {ct.giftMidAutumn&&<span style={{fontSize:10,padding:'1px 5px',borderRadius:3,background:'#fff8e1',border:'1px solid #f9d030',color:'#7a5c00'}}>中秋</span>}
    {ct.giftYearEnd  &&<span style={{fontSize:10,padding:'1px 5px',borderRadius:3,background:'#ffeaea',border:'1px solid #f89898',color:'#8b0000'}}>年節</span>}
    {ct.giftCalendar &&<span style={{fontSize:10,padding:'1px 5px',borderRadius:3,background:'#e8f4ff',border:'1px solid #7ab8f5',color:'#1a5fa8'}}>桌曆</span>}
  </div>
)
...
export default function ClientsPage(){
  ...
  const [giftFilter,setGiftFilter]=useState<GiftFilterKey|''>('')
  ...
  const openNew=()=>{isNew.current=true;setEditing(emptyClient());setContacts([emptyContact()]);setModalOpen(true)}
  ...
  const updCt=(i:number,f:keyof Contact,v:any)=>setContacts(prev=>prev.map((c,idx)=>idx===i?{...c,[f]:v}:c))
  const addCt=()=>{if(contacts.length<4)setContacts(prev=>[...prev,emptyContact()])}
  const rmCt=(i:number)=>setContacts(prev=>prev.filter((_,idx)=>idx!==i))

  const save=async()=>{
    if(!editing.name?.trim()){alert('請填寫委託單位名稱');return}
    setSaving(true)
    try{
  ...
  return(
    <div className="app">
      ...
      <div className="main">
        ...
        {/* 篩選列 */}
        <div className="filter-bar" style={{gap:8,flexWrap:'wrap',alignItems:'center'}}>
        ...
        <div className="scroll-area">
          <div style={{display:'grid',gridTemplateColumns:'88px 1fr 100px 130px 130px 80px',alignItems:'center',padding:'8px 16px',borderBottom:'1px solid var(--bd)',background:'var(--bgh)'}}>
          ...
          {loading?(
            ...
            const firstCt=allCts[0]
            const hasGift=[1,2,3,4].some(i=>(c as any)[`contact${i}GiftMidAutumn`]||(c as any)[`contact${i}GiftYearEnd`]||(c as any)[`contact${i}GiftCalendar`])
            return(
              <div key={c.id} className="client-row-wrap">
                <div className="client-hrow" style={{gridTemplateColumns:'88px 1fr 100px 130px 130px 80px'}} onClick={()=>toggleExpand(c.id)}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:6,minWidth:0}}>
                    ...
                    {hasGift&&<span style={{fontSize:11,flexShrink:0}}>🎁</span>}
                  </div>
                ...
                {expanded[c.id]&&(
                  <div className="client-expand open">
                    {(c.address||c.fax||c.notes)&&(
                      <div style={{display:'flex',gap:16,marginBottom:10,fontSize:12,color:'var(--tx2)',flexWrap:'wrap',padding:'6px 10px',background:'var(--bgh)',borderRadius:6}}>
                      ...
                      <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap'}}>
                        {allCts.map((ct,i)=>(
                          <div key={i} style={{background:'var(--bgc)',border:'1px solid var(--bd)',borderRadius:6,padding:'10px 12px',fontSize:12,minWidth:190,maxWidth:260,flex:'0 0 auto'}}>
                            ...
                            {(ct.dept||ct.title)&&<div style={{color:'var(--tx3)',fontSize:11,marginBottom:4}}>{[ct.dept,ct.title].filter(Boolean).join(' · ')}</div>}
                    ...
                    {!casesMap[c.id]?(
                      ...
                      <a key={cs.id} href={`/cases?highlight=${cs.id}`}
                        ...
                        <span><b>{cs.name}</b>{cs.caseNumber&&<span style={{color:'var(--tx3)',fontFamily:'var(--m)',fontSize:11,marginLeft:6}}>#{cs.caseNumber}</span>}</span>
        ...
        <div className="mo" style={{maxWidth:700}}>
          ...
          <div className="mo-body">
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
              ...
              {contacts.length<4&&<button className="btn btn-sm" onClick={addCt}>＋ 新增窗口</button>}
            </div>
            {contacts.map((ct,i)=>(
              <div key={i} style={{border:'1px solid var(--bd)',borderRadius:8,padding:12,marginBottom:8}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                  ...
                  {i>=1&&<button style={{fontSize:11,color:'var(--rose)',cursor:'pointer',background:'none',border:'none'}} onClick={()=>rmCt(i)}>✕ 移除</button>}
                </div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:8}}>
                  <div className="fg"><label>姓名</label><input className="fi" value={ct.name} onChange={e=>updCt(i,'name',e.target.value)}/></div>
                  <div className="fg"><label>部門</label><input className="fi" value={ct.dept} onChange={e=>updCt(i,'dept',e.target.value)}/></div>
                  <div className="fg"><label>職稱</label><input className="fi" value={ct.title} onChange={e=>updCt(i,'title',e.target.value)}/></div>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:8}}>
                  <div className="fg"><label>電話</label><input className="fi" value={ct.phone} onChange={e=>updCt(i,'phone',e.target.value)}/></div>
                  <div className="fg"><label>分機</label><input className="fi" value={ct.ext} onChange={e=>updCt(i,'ext',e.target.value)}/></div>
                  <div className="fg"><label>手機</label><input className="fi" value={ct.mobile} onChange={e=>updCt(i,'mobile',e.target.value)}/></div>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:8}}>
                  <div className="fg"><label>Email</label><input className="fi" type="email" value={ct.email} onChange={e=>updCt(i,'email',e.target.value)}/></div>
                  <div className="fg"><label>生日（月/日）</label><input className="fi" placeholder="MM/DD" value={ct.birthday} onChange={e=>updCt(i,'birthday',e.target.value)}/></div>
                  <div className="fg"><label>備註</label><input className="fi" value={ct.notes} onChange={e=>updCt(i,'notes',e.target.value)}/></div>
                </div>
                {/* 每個窗口的送禮設定 */}
                <div style={{display:'flex',gap:14,alignItems:'center',padding:'6px 8px',background:'var(--bgh)',borderRadius:5}}>
                  ...
                  {GIFTS.map(g=>(
                    <label key={g.key} style={{display:'flex',alignItems:'center',gap:4,fontSize:12,cursor:'pointer'}}>
                      <input type="checkbox" checked={!!(ct as any)[g.key]} onChange={e=>updCt(i,g.key as keyof Contact,e.target.checked)}/>
                      {g.label}
                    </label>
                  ))}
                </div>
